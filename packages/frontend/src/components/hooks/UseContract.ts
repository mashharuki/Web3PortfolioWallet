...
export const getDid = async(signer:any) => {
      ...
      const DNSContract = createContractObject(DNS.abi, DNS_ADDRESS);
      // get did info
      const result = await DNSContract.methods.dids(signer).call();
      return result;
};
...
export const getRegisterStatus = async(signer:any) => {
      ...
      const DNSContract = createContractObject(DNS.abi, DNS_ADDRESS);
      // get status info
      var status = await DNSContract.methods.isRegistered(signer).call();
      return status;
};
...
export const getVcs = async(did:any) => {
      ...
      var vcs = await DNSContract.methods.getVcs(did).call();
      return vcs;
};
